...
#define K_MAX_DELTA_TIME_US 200
...
static 		 uint32 gu32_TelegramEndPosition;
...
static Std_ReturnType lms151_read( te_ReadStatus * const pe_ReadStatus)
{
	...
	bool b_TimeoutElapsed;
	struct timespec t_clock1, t_clock2;
	uint32 u32_delta_time_us;
	uint32 u32_cpt;

    /* Initialization */
	s32_read=0;
	t_read_status=E_OK;
	b_TimeoutElapsed=K_FALSE;
	clock_gettime(CLOCK_MONOTONIC, &t_clock1);
	...
	if (NULL_PTR!=pe_ReadStatus)
	{
		*pe_ReadStatus=E_READ_NO_DATA;
		memset (gau8_TemporaryFrameReceive,0,K_FRAME_SIZE_MAX);
		...
		/*! While Timeout not elapsed and end of trame not found, search end of frame */
		while ((E_READ_FINISH!=*pe_ReadStatus)&&
				(K_FALSE==b_TimeoutElapsed)&&
				(gu32_TelegramEndPosition<K_FRAME_SIZE_MAX))
		{
			if (gau8_FrameReceive[gu32_TelegramEndPosition]!=gu8_telegram_end)
			{
//				gu32_FramePosition++;
				gu32_TelegramEndPosition++;
			}
			else
			{
				/*! if data in frame received is equal to gu8_telegram_end, set pe_ReadStatus data to E_READ_FINISH */
				*pe_ReadStatus=E_READ_FINISH;
				gu32_TelegramEndPosition=0;
			}
			clock_gettime(CLOCK_MONOTONIC, &t_clock2);
			u32_delta_time_us=(t_clock2.tv_nsec - t_clock1.tv_nsec)/1e3;
			if (u32_delta_time_us>K_MAX_DELTA_TIME_US)
			{
				b_TimeoutElapsed=K_TRUE;
			}
			else
			{
				b_TimeoutElapsed=K_FALSE;
		...
		if (gu32_TelegramEndPosition>=K_FRAME_SIZE_MAX)
		{
			t_read_status=E_NOT_OK;
		}

//		/*! Check size given is equal to data read */
//		/*! and t_error different to E_DRVTCPSOCKET_ERROR_NoError */
//		if ((s32_read<0)||
//			(E_DRVTCPSOCKET_ERROR_NOERROR!=t_error))
//		{
//			t_read_status=E_NOT_OK;
...
//			{
//				*pe_ReadStatus=E_READ_BUSY;
//				/*! size data read is positive, fill gau8_FrameReceive */
//				while ((E_READ_FINISH!=*pe_ReadStatus)&&
//						(u32_i<(uint32)abs(s32_read))&&
//						(K_FALSE==b_TimeoutElapsed))
//				{
//					if (gau8_TemporaryFrameReceive[u32_i]!=gu8_telegram_end)
//					{
//						/*! if data in frame received is not equal to gu8_telegram_end, set pe_ReadStatus data to E_READ_BUSY */
...
//					}
//					gau8_FrameReceive[gu32_FramePosition]=gau8_TemporaryFrameReceive[u32_i];
//					u32_i++;
//					gu32_FramePosition++;
//				}
//			}
		...
		(E_NOT_OK!=gt_ConfLibLidarLms151_Status))
    {
    	switch(ge_InternalStateMachine)
    	{
			...
			case E_STATE_LMS151_UNDEFINED :
												t_Manage_Status=E_NOT_OK;
										    	printf("E_STATE_LMS151_UNDEFINED \n");
												break;
													...
													case E_MODE_LOGIN:
																switch (ge_SubState)
																{
																	case E_IDLE :	/*! log into laser for parameters modifications */
																					...
																					{
																				    	printf("lms151_logIn_set E_NOT_OK \n");
																					}
																					break;
																	case E_READ :	/*! Check login command has been well sent */
																					...
																					{
																						ge_SubState=E_READ;  // TODO E_ERROR
																						t_Manage_Status=E_OK; // TODO TO BE DELETED, FOR TEST DEBUG PURPOSE ONLY
																						gu32_FramePosition=0;
																				    	printf("lms151_logIn_check E_NOT_OK \n");
																				    	printf("gau8_FrameReceive : ");
																				    	for (u32_cpt=0;u32_cpt<30;u32_cpt++)
																				    	{
																				    		printf("%c",gau8_FrameReceive[u32_cpt]);
																				    	}
																				    	printf("/n");
																					}
																					break;
													...
													case E_MODE_SET_FREQUENCY_AND_RESOLUTION:
														switch (ge_SubState)
														{
															case E_IDLE :	/*! Configure laser Frequency and resolution */
																			...
																			{
																		    	printf("lms151_FrequencyAndResolution_set E_NOT_OK \n");
																			}
																			break;
															case E_READ :	/*! Check laser frequency and resolution configuration well done */
																			...
																			else
																			{
																				...
																				t_Manage_Status=E_OK; // TODO TO BE DELETED, FOR TEST DEBUG PURPOSE ONLY
																				gu32_FramePosition=0;
																		    	printf("lms151_FrequencyAndResolution_check E_NOT_OK \n");
																		    	printf("gau8_FrameReceive : ");
																		    	for (u32_cpt=0;u32_cpt<30;u32_cpt++)
																		    	{
																		    		printf("%c",gau8_FrameReceive[u32_cpt]);
															...
															default :
																	    	...
																	    	printf("E_LIBLMS151_ERR_SET_FREQUENCY_AND_RESOLUTION \n");
																	    	t_Manage_Status=E_NOT_OK;
																			break;
														}
														break;
													case E_MODE_CONFIGURE_SCAN_DATA_CONTENT:
														switch (ge_SubState)
														{
															case E_IDLE :   /*! Configure scan data content */
																			...
																			{
																				printf("lms151_configure_ScanDataContent_set E_NOT_OK \n");
																			}
																			break;
															case E_READ : 	/*! Check scan data content configuration has been well done */
																			...
																			else
																			{
																				...
																				t_Manage_Status=E_OK; // TODO TO BE DELETED, FOR TEST DEBUG PURPOSE ONLY
																				gu32_FramePosition=0;
																				printf("lms151_configure_ScanDataContent_check E_NOT_OK \n");
																		    	printf("gau8_FrameReceive : ");
																		    	for (u32_cpt=0;u32_cpt<30;u32_cpt++)
																		    	{
																		    		printf("%c",gau8_FrameReceive[u32_cpt]);
															...
															default :
																	    	...
																	    	printf("E_LIBLMS151_ERR_CONFIGURE_SCAN_DATA_CONTENT \n");
																	    	t_Manage_Status=E_NOT_OK;
																			break;
														}
														break;
													case E_MODE_CONFIGURE_SCAN_DATA_OUTPUT:
														switch (ge_SubState)
														{
															case E_IDLE :	/*! configure laser scan data output */
																			...
																			{
																				printf("lms151_configure_ScanDataOutput_set E_NOT_OK \n");
																			}
																			break;
															case E_READ :	/*! check scan data output laser command has been well sent */
																			...
																			else
																			{
																				...
																				t_Manage_Status=E_OK; // TODO TO BE DELETED, FOR TEST DEBUG PURPOSE ONLY
																				gu32_FramePosition=0;
																				printf("lms151_configure_ScanDataOutput_check E_NOT_OK \n");
																		    	printf("gau8_FrameReceive : ");
																		    	for (u32_cpt=0;u32_cpt<30;u32_cpt++)
																		    	{
																		    		printf("%c",gau8_FrameReceive[u32_cpt]);
															...
															default :
																	    	...
																	    	printf("E_LIBLMS151_ERR_CONFIGURE_SCAN_DATA_OUTPUT \n");
																	    	t_Manage_Status=E_NOT_OK;
																			break;
														}
														break;
													case E_MODE_STORE_PARAMETERS:
														switch (ge_SubState)
														{
															case E_IDLE :   /*! Store parameters in laser eeprom */
																			...
																			{
																				printf("lms151_store_parameters_set E_NOT_OK \n");
																			}
																			break;
															case E_READ :	/* check store parameters command has been well sent */
																			...
																			else
																			{
																				...
																				t_Manage_Status=E_OK; // TODO TO BE DELETED, FOR TEST DEBUG PURPOSE ONLY
																				gu32_FramePosition=0;
																				printf("lms151_store_parameters_check E_NOT_OK \n");
																		    	printf("gau8_FrameReceive : ");
																		    	for (u32_cpt=0;u32_cpt<30;u32_cpt++)
																		    	{
																		    		printf("%c",gau8_FrameReceive[u32_cpt]);
															...
															default :
																	    	...
																	    	printf("E_LIBLMS151_ERR_STORE_PARAMETERS \n");
																	    	t_Manage_Status=E_NOT_OK;
																			break;
														}
														break;
												...
												case E_MODE_START_MEAS:
													switch (ge_SubState)
													{
														case E_IDLE :	/*! Start Measurements */
																		...
																		{
																			printf("lms151_StartMeas_set E_NOT_OK \n");
																		}
																		break;
														case E_READ :	/*! Check start measurements command has been well sent */
																		...
																		else
																		{
																			...
																			t_Manage_Status=E_OK; // TODO TO BE DELETED, FOR TEST DEBUG PURPOSE ONLY
																			gu32_FramePosition=0;
																			printf("lms151_StartMeas_check E_NOT_OK \n");
																	    	printf("gau8_FrameReceive : ");
																	    	for (u32_cpt=0;u32_cpt<30;u32_cpt++)
																	    	{
																	    		printf("%c",gau8_FrameReceive[u32_cpt]);
														...
														default :
																    	...
																    	printf("E_LIBLMS151_ERR_START_MEAS \n");
																    	t_Manage_Status=E_NOT_OK;
																		break;
													}
													break;
									...
									case E_MODE_ASK_STATUS :
											switch (ge_SubState)
											{
												case E_IDLE :	/*! Ask laser state */
																...
																{
																	printf("lms151_AskDeviceStatus_set E_NOT_OK \n");
																}
																break;
												case E_READ :   /*! Check laser ask state command has been well sent */
																...
																else
																{
																	...
																	t_Manage_Status=E_OK; // TODO TO BE DELETED, FOR TEST DEBUG PURPOSE ONLY
																	gu32_FramePosition=0;
																	printf("lms151_AskDeviceStatus_check E_NOT_OK \n");
															    	printf("gau8_FrameReceive : ");
															    	for (u32_cpt=0;u32_cpt<30;u32_cpt++)
															    	{
															    		printf("%c",gau8_FrameReceive[u32_cpt]);
												...
												default :
														    	...
														    	printf("E_LIBLMS151_ERR_ASK_STATUS \n");
														    	t_Manage_Status=E_NOT_OK;
																break;
											}
											break;
										case E_MODE_LOGOUT:
											switch (ge_SubState)
											{
												case E_IDLE :  /*! log out to run laser */
																...
																{
																	printf("lms151_logOut_set E_NOT_OK \n");
																}
																break;
												case E_READ :	/*! Check log out command has been well sent */
																...
																else
																{
																	...
																	t_Manage_Status=E_OK; // TODO TO BE DELETED, FOR TEST DEBUG PURPOSE ONLY
																	gu32_FramePosition=0;
																	printf("lms151_logOut_check E_NOT_OK \n");
															    	printf("gau8_FrameReceive : ");
															    	for (u32_cpt=0;u32_cpt<30;u32_cpt++)
															    	{
															    		printf("%c",gau8_FrameReceive[u32_cpt]);
								...
								case E_MODE_SCAN_CONTINUOUS :
										switch (ge_SubState)
										{
											case E_IDLE :	/*! Configure permanent scan */
															...
															{
																printf("lms151_Permanent_Scan_set E_NOT_OK \n");
															}
															break;
											case E_READ :   /*! check permanent scan well configured */
															...
															else
															{
																...
																t_Manage_Status=E_OK; // TODO TO BE DELETED, FOR TEST DEBUG PURPOSE ONLY
																gu32_FramePosition=0;
																printf("lms151_Permanent_Scan_check E_NOT_OK \n");
														    	printf("gau8_FrameReceive : ");
														    	for (u32_cpt=0;u32_cpt<30;u32_cpt++)
														    	{
														    		printf("%c",gau8_FrameReceive[u32_cpt]);
											...
											default :
															...
															ge_InternalStateMachine=E_STATE_LMS151_UNDEFINED;
													    	printf("E_LIBLMS151_ERR_SCAN_CONTINUOUS \n");
													    	t_Manage_Status=E_NOT_OK;
															break;
										}
										break;
