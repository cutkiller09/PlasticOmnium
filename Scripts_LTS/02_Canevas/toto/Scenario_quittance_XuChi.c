#################################################################################################################################################### 
#! les '#!' correspondent a des commentaires
#! les '#' correspondent a des mots clefs
#! les '|' correspondent a des delimitateurs
#! !!!!! TRES IMPORTANT !!!!!
#! !!!!! NE PAS METTRE  LA DATE AU FORMAT XX/YY/DDDD!!!!!
#! !!!!! REMPLACER "=" par "\x3d"  "&" par "\x26" "/" par "\x2f"!!!!
#################################################################################################################################################### 

#Donnees_Locataire
#$PRENOM_LOCATAIRE | $NOM_LOCATAIRE | $DATE_ANNIVERSAIRE | $LIEU_NAISSANCE | $NATIONALITE 
Elodie | Courel | 28 Octobre 1989 | Foix (09000) | Francaise

#Donnees_bien
#$ADRESSE_BIEN | $DESIGNATION | $TYPE 
1er \etage \a \droite, 3 rue Lafaurie 09000 Foix | un appartement de type T2, comprenant une chambre avec lit, un sejour avec cuisine integree, une salle d eau et un WC | T2

#Duree
#$JOUR_ENTREE | $MOIS_ENTREE | $ANNEE_ENTREE | $ANNEE_SORTIE_PREVUE | $DUREE_BAIL 
24 | Janvier | 2015 | 2018 | 3 

#Loyer
#$LOYER_HORS_CHARGES | $LOYER_CHARGES_COMPRISES | $CHARGES
320 | 350 | 30

#Revision_Loyer
#$INDICE_INSEE | $TRIMESTRE_INSEE
125,29 | 4eme trimestre 2014

#Donnees_Bailleur
#$IDENTITE_BAILLEUR | $ADRESSE_BAILLEUR
Sebastien Loisel | 22 rue Anatole France, 31190 Auterive

#Donnees_Quittance
#$MOIS_QUITTANCE | $ANNEE_QUITTANCE
Janvier | 2015

#Donnees_Quittance
#$MOIS_QUITTANCE | $ANNEE_QUITTANCE
Fevrier | 2015

#Donnees_Quittance
#$MOIS_QUITTANCE | $ANNEE_QUITTANCE
Mars | 2015

#Donnees_Quittance
#$MOIS_QUITTANCE | $ANNEE_QUITTANCE
Avril | 2015

#Donnees_Quittance
#$MOIS_QUITTANCE | $ANNEE_QUITTANCE
Mai | 2015

#Donnees_Quittance
#$MOIS_QUITTANCE | $ANNEE_QUITTANCE
Juin | 2015

#Donnees_Quittance
#$MOIS_QUITTANCE | $ANNEE_QUITTANCE
Juillet | 2015

#Donnees_Quittance
#$MOIS_QUITTANCE | $ANNEE_QUITTANCE
Aout | 2015

#Donnees_Quittance
#$MOIS_QUITTANCE | $ANNEE_QUITTANCE
Septembre | 2015

#Donnees_Quittance
#$MOIS_QUITTANCE | $ANNEE_QUITTANCE
Octobre | 2015

#FinTraitement 
